...
search_junctions_in_df <- function(df, junctions, wiggle_room) {
  results <- data.frame(A_C = character(), stringsAsFactors = FALSE)
...
all_results <- data.frame(File = character(), A_C = character(), BP = integer(), RI = integer(), stringsAsFactors = FALSE)
for (file in text_files) {
  cat("Searching in file:", file, "\n")  # Debugging statement
  df <- read_delim(file, delim = "\t", col_types = cols(`A_C(A_C)` = col_character()))  # Ensure A_C column is character
  
  # Check if the dataframe has the necessary column
  if (!"A_C(A_C)" %in% colnames(df)) {
    cat("File", file, "does not contain required column 'A_C(A_C)'. Skipping...\n")
    next
  }
  
  # Remove content within parentheses and keep only the first part
  df <- df %>%
    mutate(A_C = str_extract(`A_C(A_C)`, "^[^()]+")) %>%
    separate(A_C, into = c("BP", "RI"), sep = "_", convert = TRUE)  # Convert to numeric
