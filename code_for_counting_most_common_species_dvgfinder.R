...
junctions <- combined_deletions %>% select(BP,RI)
junctions$ID <- paste(junctions$BP, junctions$RI, sep = "_")
...
junctions_filtered <- junctions_filtered %>%
  filter(Count >= 2) %>%  # Click on the df to see what the max number is, and set that
...
ggplot(junctions_filtered, aes(x = RI)) +
...
junctions_filtered <- junctions_filtered %>%
  filter(RI < 3600) # Example RI here is 3,600
